...
- ✅ **Geração de Parsers:** Geração do código fonte (em Python e JavaScript) para Parsers **Recursivos Descendentes** e **Top-Down Dirigidos por Tabela**.
...
│   ├── parser_generator.py     # Geradores de Código (Recursivo/Tabela)
│   ├── derivation_tree.py      # Construção da Árvore (Wrapper Lark)
│   ├── lark_parser.py          # Conversor para EBNF do Lark
│   └── visitor.py              # Classe base do padrão Visitor
...
└── test_phase1_advanced.py     # Testes de integração rigorosos (Asserts)        # Configuração do projeto
```
...
4. **Padrão Visitor**: Na aba "Visitor", o servidor Flask recebe o código Python escrito pelo utilizador e executa-o num ambiente isolado (exec()) injetando-lhe a classe base TreeVisitor e a árvore gerada.
...
O menu lateral está dividido em 4 abas para guiar o fluxo de trabalho:
...
3. **Parsers**: Escolha o tipo de parser (Tabela ou Recursivo) e a linguagem-alvo (Python/JS). O código-fonte integral será gerado.

4. **Visitor**: Escreva uma classe Python chamada MyVisitor herdando de TreeVisitor para processar a árvore. Por exemplo:

```Python
class MyVisitor(TreeVisitor):
    def visit_Stmt(self, node):
        # Extrai dados dos filhos do nó e gera código
        return f"Encontrei uma instrução!"
```